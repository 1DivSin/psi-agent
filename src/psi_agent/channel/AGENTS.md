...
- **SDK 媒体批处理必须显式开启**：`lark-channel-sdk` 的 `SafetyConfig.media_batch.enabled` 默认是 `False`。生产构造 `FeishuChannel` 时必须传 `SafetyConfig(media_batch=MediaBatchConfig(enabled=True))`，否则用户一次选择多份简历时 SDK 不会合并源消息，批量附件协议无法可靠触发。此开关只控制消息归并，不替代 `batched_sources` 的逐源下载逻辑；多条消息逐步发送的收集协议仍由上层 workflow-input-collections 负责。
