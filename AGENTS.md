...
4. **Usage 信号（SSE 层面）**：provider 的流式 token 用量通常位于 `choices=[]` 的尾帧，
   ...
   Anthropic 流的完整 usage 可能位于 `message_start` 或 `message_delta`；内部 `prompt_tokens`
   包含未缓存输入、缓存读取和缓存创建三部分。仓库未显式设置 `cache_control` 不等于 provider
   一定没有缓存，实际行为以上游 usage 为准。传输顺序固定为 usage、可选 compaction、业务
   terminal；Session 在执行工具或发送 tool progress 前记账，避免消费者提前关闭导致漏计。
