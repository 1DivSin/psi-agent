...
def _combine_artifact_contracts(
    ...
    *,
    ...
    if (
        existing.json_type is not None
        and addition.json_type is not None
        and existing.json_type != addition.json_type
    ):
    ...
    *,
    ...
    for artifact_id, value in values.items():
        ...
        if not _artifact_type_matches(value, contract.json_type):
            raise ValueError(
                f"{context} Artifact {artifact_id!r} must be JSON {contract.json_type}, "
