...
人才库显示 14 个业务字段：姓名、评级、学历、毕业院校/背景、总分、备注、匹配岗位、匹配点、不匹配点、面试建议、面试建议理由、问题库、初审状态、简历摘要。问题库按 `1. [类别] 问题` 的固定格式显示 3–6 条；证据锚点、目的和正负信号只保存在 workspace 私有交接中。面试记录只显示姓名、目标岗位、面试前摘要、详细面试重点、风险提示、建议问题、纪要/补充、评分、定级、疑问、风险验证、结论和状态。技术 ID、评估版本和结构化 JSON 不写入飞书，而是按面试记录的 Feishu `record_id` 保存在 workspace 私有交接目录。
...
- `岗位需求`：岗位运营视图；
- `人才库`：14 字段用户可见看板，每位候选人的初审入口，也是简历评估、问题库与初审结果的事实源；
- `面试记录`：跨天维护面试过程，也是面试证据与最终录用状态的事实源；
- 汇总文档：最终结论的审计摘要，不是事实源。

由于底层尚无 update/upsert，人才库创建使用包含 `问题库` 的 12 个 AI 所有字段作为确定性指纹，排除 Human 会修改的 `备注` 和 `初审状态`；后续跨阶段一律使用 Feishu `record_id` 精确关联。重复指纹或模糊关联会 fail-closed。
