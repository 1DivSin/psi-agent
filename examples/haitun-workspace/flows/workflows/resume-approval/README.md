...
为本地测试和可重复运行，仍兼容 workspace 内的路径字符串或 `{"path":"...","name":"..."}` 描述符。不要传入未经运行器证明的附件 ID、URL 或 Base64 字段。允许格式为 PDF、DOCX、Markdown 和纯文本；每个源文件最大 50 MiB。所有文件进入 SHA-256 地址化目录并按内容去重；同一内容同时来自上传目录和 workspace 时，上传附件优先保留。原始上传文件名只保存在本地 `staged_resume_files` 运行 Artifact 中用于溯源；进入提取和评估合同的名称统一为中性的 `resume.<ext>`，防止文件名中的手机号或其他个人信息进入 `source.name`。

人才库显示 13 个业务字段：姓名、评级、学历、毕业院校/背景、总分、备注、匹配岗位、匹配点、不匹配点、面试建议、面试建议理由、初审状态、简历摘要。面试记录只显示姓名、目标岗位、面试前摘要、详细面试重点、风险提示、建议问题、纪要/补充、评分、定级、疑问、风险验证、结论和状态。技术 ID、评估版本和结构化 JSON 不写入飞书，而是按面试记录的 Feishu `record_id` 保存在 workspace 私有交接目录。
...
4. 确认 HaiTun 进程已配置 `PSI_FEISHU_APP_ID` 和 `PSI_FEISHU_APP_SECRET`，并具备目标文档、人才库和面试记录表的读取及写入权限；密钥不得写入配置、日志或提交。
...
- `岗位需求`：岗位运营视图；
- `人才库`：13 字段用户可见看板，每位候选人的初审入口，也是简历评估与初审结果的事实源；
- `面试记录`：跨天维护面试过程，也是面试证据与最终录用状态的事实源；
- 汇总文档：最终结论的审计摘要，不是事实源。

由于底层尚无 update/upsert，人才库创建使用 11 个 AI 所有字段作为确定性指纹，排除 Human 会修改的 `备注` 和 `初审状态`；后续跨阶段一律使用 Feishu `record_id` 精确关联。重复指纹或模糊关联会 fail-closed。
