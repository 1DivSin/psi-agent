...
4. A2 rereads every expected talent row by exact Feishu `record_id`, validates the complete 11-field AI-owned fingerprint, and accepts only `通过` or `不通过`.
...
The immutable input contains pre-review facts only. A2 does not trust chat text as a decision. It independently verifies the descriptor hash, local Feishu destination, assessment revisions, exact talent-record coverage, and live Human decisions before generating any interview draft.
...
One read-only Agent first validates the completed Human decisions. Then one read-only draft Agent handles each approved candidate, with workflow concurrency limited to four. Each draft iteration returns an Artifact only. After all drafts complete, a deterministic Program attaches authoritative candidate, role, talent-record, and revision identities and creates one source-ordered write batch. A separate Agent performs all Feishu interview-table searches and creates. Parallel draft Agents never write shared files or tables.
